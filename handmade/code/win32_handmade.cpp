...
int CALLBACK WinMain(HINSTANCE Instance, HINSTANCE PrevInstance, LPSTR CmdLine, int ShowCode) {
    ...
    if (RegisterClass(&WindowClass)) {
        ...
        if (Window) {
            ...
            Win32_InitDSound(Window, samplesPerSecond, samplesPerSecond * bytesPerSample);
            ...
            while (running) {
                ...
                if (SUCCEEDED(secondaryBuffer->GetCurrentPosition(&playCursor, &writeCursor))) {
                    ...
                    void *region2;
                    DWORD region2Size;
                    if (SUCCEEDED(secondaryBuffer->Lock(byteToLock, bytesToWrite, &region1, &region1Size, &region2,
                                                        &region2Size, 0))) {
